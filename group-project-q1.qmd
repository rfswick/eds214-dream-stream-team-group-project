...
k_plot <- ggplot(data_sub, aes(x = date, y = k, color = sample_id)) + 
  ...
  theme(legend.position = "none", panel.background = element_rect(fill = "white"), panel.grid = element_line(color = soft_col))  + 
  labs(y = "K (mg/L)") +
...
no3_plot <- ggplot(data_sub, aes(x = date, y = no3, color = sample_id)) + 
  geom_line(linewidth = 0.5) + 
  theme(legend.position = c(0.9, 0.6), panel.background = element_rect(fill = "white"), panel.grid = element_line(color = soft_col)) + 
...
```{r}
ggsave(here("/Users", "jpcohen", "eds_214_group_project", "eds214-dream-stream-team-group-project", "group-project-q1_files", "figs", "replicate.png"), plot = patchGrob, width = 15)
```
