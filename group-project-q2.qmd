...
pivot_test <- quality_rainfall_data %>% select(date, discharge_ft3_s, p_h, cond, po4_p, doc) %>%
...
```{r}
ggplot(data = subset(pivot_test, !is.na(value)), aes(y = value, x = date, color = variable)) + geom_point(size = 4) + facet_wrap(vars(variable))
```
