...
# and i decided to use the pivot method since apparently melt is deprecated
# ill get rid of the melt stuff later though

melt_test <- melt(quality_rainfall_data, id = "date", measure = c("discharge_ft3_s", "p_h", "cond", "po4_p", "doc"))
...
```{r}
# i was practicing graphing with the melt method
# will delete later

ggplot(data = subset(melt_test, !is.na(value)), aes(y = value, x = date, color = variable)) + geom_point() + facet_wrap(vars(variable))
```
...
ggplot(data = subset(pivot_test, !is.na(value)), aes(y = value, x = date, color = variable)) + geom_point(size = 4, show.legend = FALSE) + facet_wrap(vars(variable), strip.position = "bottom") + scale_x_date(position = "top")
```
